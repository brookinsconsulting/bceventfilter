...
{def    $events=fetch( 'content', 'list', hash(
            ...
            'sort_by', array( 'attribute', true(), 'production_event/from_time'),
            'class_filter_type',  'include',
            'class_filter_array', array( 'production_event' ),
            'main_node_only', true(),
             'attribute_filter',
            array( 'or',
                    array( 'production_event/from_time', 'between', array( sum($first_ts,1), sub($last_ts,1)  )),
                    array( 'production_event/to_time', 'between', array( sum($first_ts,1), sub($last_ts,1) )) )
                ))
